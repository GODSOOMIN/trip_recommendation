...
$rank_query="SELECT district_name, rank FROM (SELECT district_id, COUNT(*) AS rank FROM place GROUP BY district_id ORDER BY rank DESC LIMIT 5) AS c JOIN district AS d ON c.district_id=d.district_id";
$rank_result=$mysqli->query($rank_query);
$z=1;
if($x==0) {
echo '<h2> !!TOP 5!! </h2><br>';
while( $rank_row=mysqli_fetch_array($rank_result)){
   echo '('.$z.')  '.$rank_row['district_name'].'   '.$rank_row['rank'].'개<br>';
   $z++;
}
}
